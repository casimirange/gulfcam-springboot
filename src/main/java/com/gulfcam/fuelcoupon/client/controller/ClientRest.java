...
<<<<<<< HEAD
        ...
        if (!createClientDTO.getNiu().isEmpty() && iClientService.existsByNiu(createClientDTO.getNiu())) {
            return ResponseEntity.badRequest().body(new MessageResponseDto(HttpStatus.BAD_REQUEST,
...
<<<<<<< HEAD
        ...
        if (!client.getEmail().equals(createClientDTO.getEmail()) && !createClientDTO.getEmail().isEmpty() && iClientService.existsByEmail(createClientDTO.getEmail())) {
            return ResponseEntity.badRequest().body(new MessageResponseDto(HttpStatus.BAD_REQUEST,
                    messageSource.getMessage("messages.email_exists", null, LocaleContextHolder.getLocale())));
        }
