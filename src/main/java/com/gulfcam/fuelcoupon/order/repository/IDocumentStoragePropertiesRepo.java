...
public interface IDocumentStoragePropertiesRepo extends JpaRepository<DocumentStorageProperties, Long> {
    ...
    @Query(value = "Select * from order_documents where id_order = ?1 and document_type = ?2 and type_id = ?3", nativeQuery = true)
    DocumentStorageProperties checkDocumentByOrderId(Long idOrder, String docType, Long type);

    @Query(value = "Select file_name from order_documents a where id_order = ?1 and document_type = ?2 and type_id = ?3", nativeQuery = true)
