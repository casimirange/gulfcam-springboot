...
public class ProductRest {
    ...
    public ResponseEntity<?> getProductsByIdOrder(@PathVariable String idOrder,
                                                              @RequestParam(required = false, value = "page", defaultValue = "0") String pageParam,
                                                              @RequestParam(required = false, value = "size", defaultValue = ApplicationConstant.DEFAULT_SIZE_PAGINATION) String sizeParam,
        ...
        log.info("crypté order "+idOrder);
        log.info("décrypté order "+aes.decrypt(key, idOrder));
        Page<ResponseProductDTO> products = iProductService.getProductsByIdOrder(Long.parseLong(idOrder),
                Integer.parseInt(pageParam), Integer.parseInt(sizeParam), sort, order);
        jsonMapper.registerModule(new JavaTimeModule());
        Object json = jsonMapper.writeValueAsString(products);
